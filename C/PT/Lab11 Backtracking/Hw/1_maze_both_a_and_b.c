...
void baccu_traccu (int map[][m], int i, int j, int maze[][m], int shortest_sol[][m], iji route[], iji shortest_route[n*m])
{
    if( (char)maze[i][j] == 'F' )
    {
        sw=1;
    ...
    if (can_step_here(i, j, maze))
    {
        ...
        if (i < n-1)
            baccu_traccu(map, i+1, j, maze, shortest_sol, route, shortest_route);
        if (j < m-1)
            baccu_traccu(map, i, j+1, maze, shortest_sol, route, shortest_route);
        if (i > 0)
            baccu_traccu(map, i-1, j, maze, shortest_sol, route, shortest_route);
        if (j > 0)
            baccu_traccu(map, i, j-1, maze, shortest_sol, route, shortest_route);
...
int main ()
{
    ...
    baccu_traccu(map, start_i, start_j, maze, shortest_sol, route, shortest_route);
